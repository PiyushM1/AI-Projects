...
# Heuristic used: Misplaced Tiles or Hamming Distance
...
def Move(curStateInfo):
    ...
    if zy > 0 and lastMove!='R':    # swap zx, zy with zx, zy-1
        ...
        if newState not in explored:
            ...
            dh = (0 if (final_sx==zx and final_sy==zy) else 1) - (0 if (final_sx==zx and final_sy==zy - 1) else 1)      
    ...
    if zx > 0 and lastMove!='D':    # swap zx, zy with zx-1, zy
        ...
        if newState not in explored:
            ...
            # Calculate the change in heuristic only using the swapped tile
            dh = (0 if (final_sx==zx and final_sy==zy) else 1) - (0 if (final_sx==zx - 1 and final_sy==zy) else 1)
...
def FindMinimumPath(initialState,goalState):
    ...
    for i in range(4):
        for j in range(4):
            tile = curState[i*4 + j]
            if tile=='0':                                           # To ignore the blank space 
                continue
            goal = finalPositions[tile]
            h += (0 if (goal[0]==i and goal[1]==j) else 1)          # Add 1 if the current tile is misplaced
